...
def build_discriminator():
    ...
    # add the first 3D convolutional block
    ...
    # add four more 3D convolutional blocks
...
# write losses to Tensorboard after each epoch
...
def get3DImages(data_dir):
    ...
    # grab volumes corresponding to the above 10 files
    all_volumes = np.asarray([getVoxelsFromMat(f) for f in all_files], dtype=np.bool)

    return all_volumes

# retrieve voxels from .mat file
def getVoxelsFromMat(path, cube_len=64):
    voxels = io.loadmat(path)['instance'] # shape (30, 30, 30)
    voxels = np.pad(voxels, (1, 1), 'constant', constant_values=(0, 0)) # increase shape to (32, 32, 32)

    if cube_len != 32 and cube_len == 64:
        voxels = nd.zoom(voxels, (2, 2, 2), mode='constant', order=0) # convert the 3D image to a 3D image with dim (64, 64, 64)
...
if __name__ == '__main__':
    ...
    data_dir = "3DShapeNetsCode/3DShapeNets/volumetric_data/" \
    ...
    adverserial_model = Model(inputs=[input_layer], outputs=[validity])
    ...
    if MODE == 'train':
        for epoch in range(epochs):
            ...
            num_batches = int(volumes.shape[0] / batch_size)
            ...
            for i in range(num_batches):
                ...
                volumes_batch = volumes[i*batch_size: (i+1)*batch_size, :, :, :]

                # generate volumes using the generator
                ...
                if i%2 == 0:
                    ...
                    loss_fake = discriminator.train_on_batch(gen_volumes, labels_real)
                ...
                g_loss = adverserial_model.train_on_batch(z, labels_real)
                print("[INFO] g_loss: {}".format(g_loss))

                gen_losses.append(g_loss)
                dis_losses.append(d_loss)

                # every ten minibatch, generate volumes and save them
                if i%10 == 0:
                    z_sample2 = np.random.normal(0, 0.33, size=[batch_size, 1, 1, 1, z_size]).astype(np.float32)
                    generated_volumes = generator.predict(z_sample2, verbose=3)

                    for idx, generated_volumes in enumerate(generated_volumes[:5]):
                        voxels = np.squeeze(generated_volumes)
                        voxels[voxels < 0.5] = 0.
                        voxels[voxels >= 0.5] = 1. 

                        saveFromVoxels(voxels, "results/img_{}_{}_{}".format(epoch, i, idx))
